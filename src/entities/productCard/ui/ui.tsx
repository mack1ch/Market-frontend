...
export const ProductCard = ({ category }: { category: ProductCategory }) => {
        ...
        <>
            <section className={styles.layout}>
                <ul className={styles.list}>
                    <h3 className={styles.categoryStyle}>{category?.name}</h3>
                    <div style={{ display: 'flex', gap: '32px' }}>
                        <React.Fragment key={category.id}>
                            {productData
                                ?.filter((product: any) => product.category.name === category.name)
                                .map((product: any) => (
                                    <article key={product.id} className={styles.card}>
                                        <Image
                                            ...
                                            src={product.cover}
                                            width={130}
                                            onClick={() => router.push(`/admin/sale/${product.id}`)}
                                            height={130}
                                            alt="ProductCard"
                                        />
                                        <dl
                                            ...
                                            <h4 className={styles.prod_title}>{product.name}</h4>
                                        </dl>
                                        <button
                                            className={styles.btnChange}
                                            onClick={() =>
                                                router.push(`/admin/sale/change/${product.id}`)
                                            }>
                                            <svg
                                                xmlns="http://www.w3.org/2000/svg"
                                                width="18"
